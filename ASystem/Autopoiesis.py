...
class Autopoiesis():
    ...
    def Update(self, rows, cols, Blocks):
        ...
        self.aveIntegrity=(self.aveIntegrity*79+self.Integrity(Blocks))/80
        self.cooling-=1
    ...
    def Dying(self, material, Blocks):
        ...
        for item in self.Boundary:
            Blocks[item[0]][item[1]].ions[material] *= 0.97

    def Integrity(self, Blocks):
        """
        Calculates ASystem integrity
        :param Blocks: Environment
        :return: Integrity
        """
        Material = 0
...
class AutopoiesisRecord():
    def __init__(self,Time):
        self.id = -1
        self.Integrity = np.zeros(Time)
        self.size = np.zeros(Time)
        self.locationx =np.zeros(Time)
        self.locationy = np.zeros(Time)
